...
const hookEvents = () => {
  const contacts = document.querySelectorAll('.btn_modal_contact');
  contacts.forEach(a => {
...
const createContactForm = () => {
  const form = Me.createDOM('form', ['form', 'modal__form', 'modal__form_contact', 'width-100'], {
    'action': ''
  });
...
const window = Me.createDOM('div', ['modal', 'modal_contact']);

const windowTitle = Me.createDOM('h2', ['h2', 'modal__title']);
windowTitle.innerText = 'Contact us';
const windowForm = createContactForm();
